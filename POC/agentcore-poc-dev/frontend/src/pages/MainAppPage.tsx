...
function MainAppPage() {
  ...
  useEffect(() => {
    const loadHistory = async () => {
      try {
        const history = await apiClient.getChatHistory(10, conversationId);
        if (history && history.length > 0) {
          ...
          // Ensure oldest messages first (top of chat), newest last (bottom)
  ...
  return (
    <AppLayout
      content={
        <SpaceBetween size="l">
          <Header
            description="Agentic root cause analysis for observability — Powered by Comprinno Technologies"
            variant="h1"
          >
