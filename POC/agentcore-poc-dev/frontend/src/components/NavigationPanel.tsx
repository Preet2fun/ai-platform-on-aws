...
export function NavigationPanel() {
  ...
  const fetchAccounts = async () => {
    ...
    try {
      ...
      // Auto-refresh any expired accounts
      for (const acc of accountList) {
        if (acc.status === 'expired' && acc.id !== 'default') {
          try {
            await apiClient.refreshAccount(acc.id);
          } catch (e) {
            console.warn(`Auto-refresh failed for ${acc.id}:`, e);
          }
        }
      }
      // Re-fetch to get updated status after refresh
      if (accountList.some((acc: any) => acc.status === 'expired' && acc.id !== 'default')) {
        const refreshedList = await apiClient.getAccounts();
        setAccounts(refreshedList);
      }

      // Set default selected account if none selected
      if (!selectedAccount && accountList.length > 0) {
        const defaultAccount = accountList.find((acc: any) => acc.id === 'default') || accountList[0];
