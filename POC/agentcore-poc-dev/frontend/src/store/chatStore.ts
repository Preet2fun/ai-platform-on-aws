...
interface ChatState {
  ...
  clearMessages: () => void;
  /** Load an existing conversation by ID (restores messages from history). */
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  addMessage: (message) =>
    set((state) => {
      // Prevent duplicate messages (same content + sender within last 2 messages)
      const lastMessages = state.messages.slice(-2);
      const isDuplicate = lastMessages.some(
        (m) => m.sender === message.sender && m.content === message.content
      );
  ...
  setMessages: (messages) => {
    // Deduplicate by content + sender (backend may return duplicates from DynamoDB)
  ...
  loadConversation: (conversationId) => set({
    messages: [],
    inputValue: '',
    conversationId,
    isStreaming: false,
