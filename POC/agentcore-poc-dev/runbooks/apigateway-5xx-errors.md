...
## Quick Decision
- If 502 (Bad Gateway) → backend integration is returning malformed responses
- If 503 (Service Unavailable) → backend is down or throttled
- If 504 (Timeout) → backend is too slow, increase integration timeout (max 29s)
- If intermittent → check CloudWatch Logs for the API stage
