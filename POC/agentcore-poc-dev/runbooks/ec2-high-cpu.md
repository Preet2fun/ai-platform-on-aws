...
## Quick Decision
- If a single process is consuming CPU → identify and restart the process
- If all instances are saturated → scale out (add instances to ASG)
- If it correlates with a deployment → rollback the recent change
- If gradual increase over days → right-size to a larger instance type
