...
## Quick Decision
- If all targets unhealthy → application is down, check ECS tasks / EC2 instances
- If some targets unhealthy → likely a failed deployment on subset of targets
- If health check path returns non-200 → fix the application health endpoint
- If new targets stay unhealthy → check security group allows ALB → target on health check port
