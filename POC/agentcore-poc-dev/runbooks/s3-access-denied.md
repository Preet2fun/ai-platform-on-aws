...
## Quick Decision
- If a specific principal is denied → check bucket policy + IAM policy intersection
- If all access denied after a change → revert the bucket policy
- If cross-account access → verify both the bucket policy AND the caller's IAM allow the action
- If KMS error → ensure the caller has kms:Decrypt on the bucket's KMS key
