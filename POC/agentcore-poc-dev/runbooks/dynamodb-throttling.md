...
## Quick Decision
- If billing mode is already PAY_PER_REQUEST → throttling is from account-level limits, contact AWS Support
- If this is a one-time spike → increase provisioned capacity temporarily
- If this is a recurring pattern → switch to on-demand or enable auto-scaling
- If specific keys are hot → redesign partition key strategy
