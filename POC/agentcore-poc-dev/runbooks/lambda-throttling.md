...
## Quick Decision
- If hitting account-level concurrency (1000 default) → request limit increase via AWS Support
- If a single function is throttled → increase its reserved concurrency
- If downstream service is overloaded → add backoff/retry in the function code
- If triggered by a burst → enable provisioned concurrency for predictable load
