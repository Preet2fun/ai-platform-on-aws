...
## Quick Decision
- If tasks are crashing in a loop → check container logs first (`aws logs` for the task)
- If image pull failed → verify ECR image exists and task role has ecr:GetAuthorizationToken
- If resource exhaustion → increase task CPU/memory in task definition
- If health check failing → check the health check path and container startup time
