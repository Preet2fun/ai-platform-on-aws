...
## Quick Decision
- If connections at max → application is leaking connections (not closing after use)
- If gradual increase → enable connection pooling (RDS Proxy or application-level)
- If spike correlates with traffic → scale to a larger instance class
- If idle connections dominating → reduce connection timeout in application config
