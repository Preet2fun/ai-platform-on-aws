...
class AgentCoreStack(Stack):
    ...
    def __init__(self, scope: Construct, id: str, environment_name: str = "dev", **kwargs):
        super().__init__(scope, id, **kwargs)

        # Environment name — resource-name prefix + Environment tag value (the tag is
        # applied stack-wide from app.py). Stored for consistency with the other stacks.
        #
        # NAMING: The AgentCore Memory and Gateway are created imperatively via boto3
        # (not CloudFormation). Their names are environment-prefixed here to match the
        # Terraform convention, and this MUST stay in sync with deploy.sh, which
        # creates/looks up the same resources by the same prefixed names:
        #   - Memory:  "${environment}_msp_assistant_memory"  (underscore — AgentCore
        #              names allow only [a-zA-Z0-9_])
        #   - Gateway: "${environment}-msp-assistant-gateway"  (hyphen — gateway names
        #              allow hyphens; matches GATEWAY_NAME in deploy.sh)
        # The gateway execution role ("msp-gateway-execution-role") is still created by
        # deploy.sh with a fixed name and referenced here as-is.
        ...
        a2a_arns = {
            ...
            'investigator': self.node.try_get_context("investigator_a2a_arn") or "",
        }
        ...
        self.resources['investigator_a2a_arn'] = a2a_arns['investigator']
    ...
    def _create_memory(self) -> dict:
        ...
        memory_name = f"{self.environment_name}_msp_assistant_memory"
    ...
    def _create_gateway(self) -> dict:
        ...
        gateway_name = f"{self.environment_name}-msp-assistant-gateway"
