...
class FrontendStack(Stack):
    ...
    def __init__(self, scope: Construct, id: str, api_url: str, alb_dns: str, cognito_config: dict, environment_name: str = "dev", **kwargs):
        super().__init__(scope, id, **kwargs)

        # Environment name — resource-name prefix + Environment tag value (the tag is
        # applied stack-wide from app.py). The S3 bucket and CloudFront distribution are
        # created by the CloudFrontToS3 Solutions Construct, which does not expose explicit
        # name props; we apply "${environment}-<resource>" Name tags to them below.
        self.environment_name = environment_name

        def _name(suffix: str) -> str:
            """Build an environment-prefixed resource name (e.g. "dev-frontend-bucket")."""
        ...
        # Apply "${environment}-<resource>" Name tags to the auto-created bucket and
        # distribution to match the Terraform naming convention.
        Tags.of(cloudfront_s3.s3_bucket).add("Name", _name("frontend-bucket"))
        Tags.of(cloudfront_s3.cloud_front_web_distribution).add("Name", _name("frontend-cdn"))
