...
from stacks.vpc_construct import ThreeTierVpc
...
class BackendStack(Stack):
    ...
    def __init__(self, scope: Construct, id: str, agentcore_resources: dict, environment_name: str = "dev", **kwargs):
        super().__init__(scope, id, **kwargs)

        # Environment name — resource-name prefix + Environment tag value, matching the
        # Terraform VPC module convention ("${environment}-<resource>"). Threaded in from
        # app.py (CDK context "environment", default "dev").
        self.environment_name = environment_name

        def _name(suffix: str) -> str:
            """Build an environment-prefixed resource name (e.g. "dev-msp-assistant-api")."""
            return f"{environment_name}-{suffix}"

        # 1. ECR Repository for backend image
        # Import existing repository created by deploy.sh (not creating to avoid conflicts).
        # Name is env-prefixed and MUST match ECR_REPO_NAME in deploy.sh.
        ecr_repo = ecr.Repository.from_repository_name(
            self, "BackendRepository",
            repository_name=_name("msp-assistant-backend")
        )
        
        # 2. DynamoDB Table for async chat state (multi-container support)
        chat_table_name = _name("msp-assistant-chat-requests")
        ...
        # 2b. DynamoDB Table for conversation history (list/create/delete in routes.py).
        # Key schema matches the backend: partition key user_id, sort key conversation_id.
        # Without this table the /conversations endpoints silently return empty (the
        # handlers swallow the AccessDenied/NotFound), so history never persists.
        conversations_table_name = _name("msp-assistant-conversations")
        ...
        task_role = iam.Role(self, "BackendTaskRole",
            role_name=_name("backend-task-role"),
            assumed_by=iam.ServicePrincipal("ecs-tasks.amazonaws.com"),
            description="Role for ECS tasks to invoke AgentCore services"
        )
        
        # Grant DynamoDB access for async chat state + conversation history
        chat_table.grant_read_write_data(task_role)
        conversations_table.grant_read_write_data(task_role)
        
        # AgentCore permissions (thin orchestration layer) — scoped to this account's resources
        ...
        # AgentCore control plane (list/describe runtimes) — scoped to this account
        task_role.add_to_policy(iam.PolicyStatement(
            effect=iam.Effect.ALLOW,
            actions=[
        ...
        # STS (cross-account assume role for tenant accounts, caller identity)
        task_role.add_to_policy(iam.PolicyStatement(
            effect=iam.Effect.ALLOW,
        ...
        task_role.add_to_policy(iam.PolicyStatement(
            ...
            resources=["arn:aws:iam::*:role/MSP-*-Role"]  # any tenant account, only our role pattern
        ))
        ...
        user_pool = cognito.UserPool(self, "UserPool",
            user_pool_name=_name("msp-assistant-users"),
        ...
        user_pool_domain = user_pool.add_domain("CognitoDomain",
            cognito_domain=cognito.CognitoDomainOptions(
                domain_prefix=f"{environment_name}-msp-assistant-{self.account}"
            )
        )
        
        # 5. Three-tier VPC — exact CDK port of the Terraform VPC module:
        #   - 3 subnets per tier across 3 AZs (public / private-app / private-db)
        #   - exactly 3 shared per-tier route tables (public→IGW, app→NAT, db→isolated)
        #   - single NAT gateway, single IGW, exact CIDRs and ${env}-<resource> naming
        #   - CloudWatch monitoring/logs interface endpoints + VPC flow logs
        # S3 gateway + ECR endpoints are added by AlbToFargate; SSM endpoints omitted.
        network = ThreeTierVpc(self, "Network",
        ...
        # 6. ECS Fargate + ALB (AWS Solutions Construct)
        # Uses the explicit VPC above via existing_vpc_obj (no auto-created VPC).
        ...
        alb_fargate = AlbToFargate(self, "BackendService",
            ...
            existing_vpc=network.vpc,  # Explicit three-tier VPC (matches Terraform module)
            # Explicit env-prefixed names (Solutions Construct otherwise auto-generates
            # hashed names). Passed as plain dicts (not typed L2 props) because the
            # construct injects the VPC itself and rejects a vpc key in these props,
            # while the typed *Props classes require vpc as a constructor arg.
            # ALB name must be <=32 chars, alphanumeric + hyphens ("dev-alb" is fine).
            cluster_props={"cluster_name": _name("ecs-cluster")},
            load_balancer_props={
                "internet_facing": True,
                "load_balancer_name": _name("alb"),
            },
            container_definition_props=ecs.ContainerDefinitionOptions(
                ...
                environment={
                    ...
                    # DynamoDB for conversation history (matches settings.CONVERSATIONS_TABLE)
                    ...
                    "INVESTIGATOR_A2A_ARN": agentcore_resources.get('investigator_a2a_arn', ''),
                },
            ...
            fargate_task_definition_props=ecs.FargateTaskDefinitionProps(
                family=_name("msp-assistant-task-definition"),
                task_role=task_role,
        ...
        # The ECS cluster name and task-def family are set via cluster_props /
        # fargate_task_definition_props above. The ALB name is not reliably honored
        # through load_balancer_props on this Solutions Construct, so set it directly
        # on the underlying CfnLoadBalancer (escape hatch). ALB name: "${env}-alb"
        # (<=32 chars, alphanumeric + hyphens).
        cfn_alb = alb_fargate.load_balancer.node.default_child
        cfn_alb.add_property_override("Name", _name("alb"))

        # Force ECS tasks into the private-app (NAT-egress) subnets.
        # AlbToFargate places Fargate tasks in the VPC's Isolated subnets by default;
        # in our three-tier VPC those are private-db (no NAT route). Tasks there cannot
        # reach the Cognito JWKS endpoint to verify JWTs, so authenticated requests hang
        # until the 29s API Gateway timeout (504). Passing vpc_subnets via
        # fargate_service_props is not honored by this Solutions Construct, so override
        # the service's subnet list directly (escape hatch) with the private-app subnets,
        # which route 0.0.0.0/0 through the NAT gateway.
        cfn_service = alb_fargate.service.node.default_child
        cfn_service.add_property_override(
            "NetworkConfiguration.AwsvpcConfiguration.Subnets",
            network.private_app_subnet_ids,
        )

        # Also apply matching Name tags (cosmetic, aligns with Terraform tag convention).
        Tags.of(alb_fargate.service.cluster).add("Name", _name("ecs-cluster"))
        Tags.of(alb_fargate.service).add("Name", _name("ecs-service"))
        Tags.of(alb_fargate.load_balancer).add("Name", _name("alb"))
        ...
        api = apigw.RestApi(self, "BackendAPI",
            rest_api_name=_name("msp-assistant-api"),
