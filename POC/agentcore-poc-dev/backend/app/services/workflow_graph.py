...
import asyncio
import boto3
from app.core.langgraph_client import AgentCoreClient, get_agentcore_client
...
class CloudWatchJiraKBRemediationGraph:
    ...
    async def approve_workflow(
        ...
        try:
            ...
            agentcore = get_agentcore_client(region=settings.AWS_REGION)
...
DO NOT ask for clarification. DO NOT ask for the project key. The project key is "{settings.JIRA_PROJECT_KEY}".
...
Execute the tool call NOW and return the ticket key and URL."""
            ...
            result = await agentcore.invoke_runtime(
                ...
                payload={
                    ...
                    "jira_project_key": settings.JIRA_PROJECT_KEY  # Explicit parameter for Supervisor
                }
            )
            ...
            jira_domain = (settings.JIRA_URL or "").rstrip("/")
...
If NO executable remediation commands found, output: NO_STEPS_FOUND"""
            ...
            # GUARD_MODE ("demo" or "production") is loaded from config.settings and controls
            # which extra flags (e.g. --dry-run) are required in production mode.
            policy_approved, policy_reason = validate_steps(steps, mode=settings.GUARD_MODE)
...
If a command cannot be reversed, output: <rollback>MANUAL: describe original state before applying</rollback>"""
            ...
            if settings.GUARD_MODE == "production":
...
Execute ALL steps in sequence. Do not skip any step."""
    ...
    async def approve_verification(
        ...
        try:
            ...
            for attempt in range(1, settings.VERIFICATION_MAX_RETRIES + 1):
                logger.info(f"[VERIFICATION] Attempt {attempt}/{settings.VERIFICATION_MAX_RETRIES}: checking alarm state")
                ...
                if attempt < settings.VERIFICATION_MAX_RETRIES:
                    logger.info(f"[VERIFICATION] Waiting {settings.VERIFICATION_RETRY_DELAY_SECONDS}s before retry...")
                    await asyncio.sleep(settings.VERIFICATION_RETRY_DELAY_SECONDS)

            # Exhausted retries
            logger.warning(f"[VERIFICATION] Alarm '{alarm_name}' did not return to OK after {settings.VERIFICATION_MAX_RETRIES} attempts")
            return {
                "success": True,
                "response": f"Alarm `{alarm_name}` is still in **{current_state}** state after {settings.VERIFICATION_MAX_RETRIES} checks ({settings.VERIFICATION_RETRY_DELAY_SECONDS}s apart). Manual investigation may be needed.",
                "agent_type": "verification",
